...
class NumMod(loader.Module):
    "Заражает по реплаю. Moded by @enicaaa"
    ...
    async def client_ready(self, client, db):
        ...
        if not self.db.get("HZNumMod", "exUsers", False):
            self.db.set("HZNumMod", "exUsers", [])
        if not self.db.get("HZNumMod", "infList", False):
            self.db.set("HZNumMod", "infList", {})
...
#
# Ебалка
    async def ebicmd(self, message):
        """ .ebi {args1} {args2 OR reply} \nВызови команду, чтобы просмотреть аргументы."""
        args = utils.get_args_raw(message)
        reply = await message.get_reply_message()
        filter_and_users = self.db.get("DANumMod", "ebi", {'users': [], 'filter': None, 'status': False})
        if not args:
            return await utils.answer(message, f"-sU --- добавить|удалить юзеров, на которых будет триггериться фильтр(ид|реплай).\n[{', '.join(list('<code>' + i + '</code>' for i in filter_and_users['users']))}]\n-sF --- установить фильтр. Допустим кусай.\n<code>{filter_and_users['filter'] if filter_and_users['filter'] else '❌Не установлен.'}</code>\n-t --- запустить|остановить.\n<b>{'✅Запущен' if filter_and_users['status'] else '❌Остановлен'}.</b>\n\nДопустимые имена фильтра:\n[фильтр] (бей|еби|кусь|кусай|уеби|зарази[ть]) (1-10) ((@id|user)|link(даже полный линк ид'а))\nИгнор регистра!!")
        args = args.split(' ', maxsplit=1)
        if len(args) == 1 and not reply and args[0] != '-t':
            return await utils.answer(message, '❌ Нет 2 аргумента и реплая.')
        elif args[0] == '-sU':
            ...
            return self.db.set("DANumMod", "ebi", filter_and_users)
        elif args[0] == '-sF':
            try:
                filter_and_users['filter'] = args[1].lower().strip()
                self.db.set("DANumMod", "ebi", filter_and_users)
                return await utils.answer(message, f"✅ Фильтр ~~~ <code>{args[1]}</code> ~~~ успешно установлен!")
            except:
                return await utils.answer(message, "Где 2 аргумент❓")
        elif args[0] == '-t':
    ...
    async def watcher(self, message):
        ...
        text = message.raw_text.lower()
        if not text.startswith(filter_and_users['filter']): return
        send_mes = re.search(r"(?P<z>бей\s|кусь\s|кусай\s|еби\s|уеби\s|зарази[ть]{,2}\s)(?P<lvl>[1-9]?[0]?\s)?(?P<link>@[0-9a-z_]+|(?:https?://)?t\.me/[0-9a-z_]+|tg://openmessage\?user_id=(?P<id>[0-9]+))", text)
        if send_mes:
...
# Жертвы
    ...
    async def zhertvycmd(self, message):
        """ .zhertvy {args1} {args2 OR reply} \nВызови команду, чтобы просмотреть аргументы."""
        args = utils.get_args_raw(message)
        reply = await message.get_reply_message()
        filter_and_users = self.db.get("HZNumMod", "zhertvy", {'users': [], 'filter': None, 'status': False})
        if not args:
            return await utils.answer(message, f"-sU --- добавить|удалить юзеров, на которых будет триггериться фильтр(ид|реплай).\n[{', '.join(list('<code>' + i + '</code>' for i in filter_and_users['users']))}]\n-sF --- установить фильтр. Допустим жертвы.\n<code>{filter_and_users['filter'] if filter_and_users['filter'] else '❌Не установлен.'}</code>\n-t --- запустить|остановить.\n<b>{'✅Запущен' if filter_and_users['status'] else '❌Остановлен'}.</b>\n\nДопустимые имена фильтра:\n[фильтр] (жертв[ыу]|жерт[ва])\nИгнор регистра!!")
        args = args.split(' ', maxsplit=1)
        if len(args) == 1 and not reply and args[0] != '-t':
            return await utils.answer(message, '❌ Нет 2 аргумента и реплая.')
        elif args[0] == '-sU':
            ...
            if user_id in filter_and_users['users']:
                ...
                return await utils.answer(message, '❌ Превышен лимит в 5 юзеров.')
            return self.db.set("HZNumMod", "zhertvy", filter_and_users)
        elif args[0] == '-sF':
            try:
                filter_and_users['filter'] = args[1].lower().strip()
                self.db.set("HZNumMod", "zhertvy", filter_and_users)
                return await utils.answer(message, f"✅ Фильтр ~~~ <code>{args[1]}</code> ~~~ успешно установлен!")
            except:
                return await utils.answer(message, "Где 2 аргумент❓")
        elif args[0] == '-t':
