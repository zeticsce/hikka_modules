...
class PingerMod(loader.Module):
	...
	async def ppingcmd(self, message):
		"""Пингует себя или бота, если указать его юзернейм аргументом и если ты сам не бот"""
		...
		if r and (message.entities and isinstance(message.entities[0], mem)):
			try:
				if (await message.client.get_entity(r[0][1:])).bot:
					if self.me.bot:
						return await utils.answer(message, "Ты и есть <b>бот</b>, тебе нельзя писать себе подобным!")
					entity = r[0]
				else:
					await utils.answer(message, f'<b>{r[0][1:]}</b> не является ботом')
					return
			except:
				await utils.answer(message, f'<b>{r[0][1:]}</b> не является ботом либо не существует')
				return
		if self.me.bot:
...
<code>mdev:</code> {str(stdev(ping_data))[0:5]}"""
		...
		entity = ('' if entity in ['me', 'me_bot', 'me_bot_twink'] else f' <b>{entity[1:]}</b>')
		await utils.answer(message, f"[{ok}]{entity}\n{ping}")
		for i in ping_msg:
			await i.delete()
		for i in ping_ans:
			await i.delete()
	...
	async def pingsetcmd(self, message):
		""".pingset .samples <число> - кол-во попыток пинга
		.pingset .timeout <число> - время ожидания ответа в секундах
		Установка параметров для пинга"""
		r = utils.get_args(message)
		if not r or len(r) == 1 or len(r) > 2 or r[0] not in [".samples", ".timeout"]:
			await utils.answer(message, f'Читай <code>.help {self.strings["name"]}</code>')
			return
		if r[0] in [".samples", ".timeout"]:
			if r[1].isnumeric():
				...
				await utils.answer(message, f"Параметр <b>{r[0]}</b> установлен на <i>{s}</i>")
			else:
				await utils.answer(message, f'Параметр <code>{r[0]}</code> должен быть целым числом')
			
async def ppping(message, entity, samples, timeout):
	await utils.answer(message, "<i>пингуем...</i>")
	ping_msg = []
	ping_data = []
	ping_ans = []
	if entity not in ['me', 'me_bot', 'me_bot_twink']:
		...
		for _ in range(samples):
			start = datetime.now()
			chat = entity if entity == 'me' else (message.to_id if entity == 'me_bot' else message.sender_id)
			msg = await message.client.send_message(chat, "ping")
			end = datetime.now()
			duration = (end - start).microseconds / 1000
			ping_data.append(duration)
			ping_msg.append(msg)
