...
###################################
        ...
        for i in args.split(' '):
            if '-' in i:
                ...
                try:
                    ...
                    await message.reply('❌ Используй правильно функцию "от-до".')
                    return
            else:
                list_args.append(i)
        a = reply.text
    ...
    async def искcmd(self, message):
        ...
        exlist.append(args)
        self.db.set("NumMod", "exUsers", exlist)
        await message.edit(f'✅ Пользователь <code>{args}</code> добавлен в исключения.')
    ...
    async def довcmd(self, message):
        ...
        if not args:
            return await utils.answer(
                ...
                f"<b><ник> <code>жертвы</code> | <code>ежа</code><b>\n"
                f"<b><ник> <code>болезни</code> | <code>бол</code>\n"
                f"<b><ник> <code>цен</code>аз\n"
                f"<b><ник> <code>лаб</code>уа | <code>статы</code>\n"
                f"<b><ник> <code><b><code>поставь вирусы</code></b> | <b><code>уведы</code></b>\n"
                f"<b><ник> <code>убери вирусы</code>\n\n"
                f"<b>✅ Апгрейд навыков:</b>\n"
